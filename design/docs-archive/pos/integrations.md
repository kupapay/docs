...
Bono Pay supports a spectrum of integrations that feed fiscal data from the POS/merchant stack to auditors, accountants, and regulators. The integration priority below keeps the most sensitive, trusted channels at the top of the stack.
...
**Authentication:** Secure upload into Bono Pay Cloud via SFTP/HTTPS using dedicated API keys and IP allow lists; the cloud service validates that each row maps to an existing sealed invoice before accepting it.  
**Example CSV snippet:**
```
...
**Authentication:** HMAC signature (`X-Bono Pay-Signature`) calculated over the payload with a shared secret per subscriber; receivers must verify the signature before acting.  
**Example payload:**
```json
{
...
**Data format:** Plugin POSTs JSON to Bono Pay Cloud with order-level data (`order_id`, `customer`, `line_items`, `shipping`, `payments`) plus mapping to canonical invoice fields; the cloud service in turn calls the local fiscal service/USB device before streaming the sealed response back into the plugin.  
...
## 6. ERP connectors (Odoo, SAP)
**Target users:** Enterprise customers connecting their ERP suites to Bono Pay’s fiscal engine for batch invoicing, advanced inventory control, and audit exports.  
...
**Authentication:** Mutual TLS between the ERP connector and Bono Pay Cloud or token-based authentication issued per `outlet_id`; connectors also carry the `device_id` of the linked USB device for traceability.  
**Example payload:**
```json
{
