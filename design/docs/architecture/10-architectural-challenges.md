# 10 Architectural Challenges for Bono Pay

This document outlines ten critical architectural, regulatory, and security challenges facing the Bono Pay system. These questions are designed to stress-test the current design assumptions, particularly regarding offline capabilities, regulatory compliance (Arrêté 033), and cryptographic integrity.
...
2. **The Fiscal Extension:** Instead of storing this key in the vulnerable PWA's `localStorage`, it is stored inside a dedicated Bono Pay Chrome Extension. The extension runs in an isolated sandbox, immune to XSS attacks on the main website.
3. **Silent Auto-Signing:** The cashier unlocks the extension once per shift with a PIN. The PWA sends invoice payloads to the extension, which silently auto-signs them (only if the request originates from the trusted `pos.bonopay.cd` domain) and returns the signature. This provides hardware-level isolation with zero per-transaction UX friction.
...
**Context:** If thousands of merchants in Kinshasa lose internet for 24 hours, they will queue millions of invoices locally. When connectivity is restored, all clients will attempt to sync simultaneously. This "thundering herd" could DDoS the Bono Pay Cloud API, the Cloud Signing Service, and the DGI's MCF endpoints.
