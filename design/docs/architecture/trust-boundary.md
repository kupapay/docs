...
Bono Pay positions the **Cloud Signing Service (HSM-backed)** as the root trusted fiscal authority. However, to support offline retail compliance, the trust boundary includes a **Semi-Trusted Zone** for clients holding valid Delegated Credentials.

Web dashboards, API consumers, and SDKs generally live in the untrusted zone; they author canonical payloads (`merchant_nif`, `outlet_id`, `pos_terminal_id`, `client`, `tax_groups`, `totals`, `payments`, `timestamp`) and deliver them to Bono Pay Cloud for validation, tax calculation, signing, and ledger storage. 

When operating in **Delegated Offline Mode**, a POS terminal equipped with the Bono Pay Fiscal Extension acts as a bounded, semi-trusted signer. It uses a short-lived Verifiable Credential to generate security elements locally, which are later reconciled with the Cloud Ledger.
...
4. **Payment metadata & tracing headers** — Each request carries `X-BonoPay-Merchant-ID`, `X-BonoPay-Outlet-ID`, optionally `X-BonoPay-User-ID`/`X-BonoPay-Source`, and other telemetry.
...
| --- | --- |
| The Cloud Signing Service (HSM) is the root authority for fiscal numbers, auth codes, timestamps, and QR payloads. | The HSM + Monotonic Counter Manager run inside Bono Pay Cloud. |
