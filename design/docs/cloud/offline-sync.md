...
Every invoice issued in the DRC must survive hostile networks. However, Arrêté 033 strictly prohibits issuing "unsigned drafts" to customers. Bono Pay solves this using the **Delegated Offline Token Architecture (Phase 1.5)**. 
...
When connectivity returns, the POS sync worker submits queued payloads **in sequential order** to the Bono Pay Cloud API:
