...
- **Missing / invalid:** Without a DEF ID the invoice cannot be tied to a certified device, so the DGI and KutaPay cloud must mark it non-compliant and block further uploads.
...
```mermaid
flowchart TB
  subgraph receipt["Mock receipt (KutaPay fiscalized)"]
