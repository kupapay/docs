...
WHEN an invoice is fiscalized, THE SYSTEM SHALL be able to produce the mandated Z, X, and A reports plus the audit export so auditors and the DGI can reconcile every journal entry without reissuing or deleting invoices. The reports feed off Bono Pay Cloud’s fiscal ledger—the hash-chained record the Cloud Signing Service (HSM) maintains after fiscalization—include the HSM-generated security elements, and are available whether the client is online or offline.
...
- **Trigger:** At the end of a business day (or whenever the merchant requests a closure) AFTER the last fiscalization event has been recorded in the cloud ledger for that day.
- **Content:**
...
- `operator_id`, `cashier_id`, `last_nonce`, `ledger_hash`.
- `security_elements`: `fiscal_number_range`, `auth_codes`, `timestamp`, `qr_payload` digest produced by the Cloud Signing Service.
- Z report metadata (generation timestamp, produced by Cloud Signing Service or dashboard request).
...
- **Purpose:** Provide a mid-shift snapshot that lets operators and inspectors verify that the cloud ledger is still accruing invoices in a consistent manner without waiting for a full day.
...
- `device_status`: queue depth, `last_sync_timestamp`, and any warnings emitted by the sync agent when invoices remain pending.
...
- **Purpose:** Deliver the full append-only cloud fiscal ledger (sales, voids, refunds, resets) to DGI for deep reconciliation or historical auditing.
...
- **Content:**
  ...
  - Include `report_generated_at`, `fiscal_authority_id`, `ledger_hash` for verification.
...
1. The Cloud Signing Service (HSM) appends each fiscalized invoice to the hash-chained ledger with the required security elements.
2. Periodic schedulers (cloud sync agent or dashboard request) sample the ledger and produce Z/X/A records. Z reports roll daily, X reports roll on demand, A reports can stream their lines.
3. Audit export reads every ledger entry in sequence order, attaches `prev_hash` and `signature`, and streams chunks to prevent timeouts.
4. Reports include the canonical security elements (`fiscal_authority_id`, fiscal number ranges, auth codes) so the DGI can verify without contacting the client applications.
5. All reports are signed by the Cloud Signing Service or the cloud agent that gathered the ledger; offline generation may be cached until connectivity returns.
